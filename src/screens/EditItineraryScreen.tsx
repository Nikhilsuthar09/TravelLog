...
export default function EditItineraryScreen() {
  ...
  const handleSave = () => {
    if (useStructuredFormat) {
      ...
      const textVersion = days
        .map((day) => {
          const dayHeader = `Day ${day.dayNumber} - ${new Date(
            day.date
          ).toLocaleDateString()}`;
          const activities = day.activities
            .map((activity) => {
              const timeStr = activity.startTime
                ...
                : "";
              return `• ${timeStr} ${timeStr ? "| " : ""}${activity.title}${
          ...
          <View style={styles.modalContent}>
            ...
            <ScrollView style={styles.modalScrollView}>
              ...
              >
                <Text style={styles.dropdownButtonText}>
                  Priority:{" "}
                  {activityPriority.charAt(0).toUpperCase() +
                    activityPriority.slice(1)}
                </Text>
                <Feather name="chevron-down" size={18} color={COLORS.text} />
              </TouchableOpacity>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    setactivityStatus(
                      item.value as "planned" | "confirmed" | "completed"
                    );
